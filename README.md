About pygypsy-feedstock
=======================

Feedstock license: [BSD-3-Clause](https://github.com/conda-forge/pygypsy-feedstock/blob/main/LICENSE.txt)
...
Once the `conda-forge` channel has been enabled, `pygypsy` can be installed with `conda`:
...
```
mamba install pygypsy
```

It is possible to list all of the versions of `pygypsy` available on your platform with `conda`:
...
```
mamba search pygypsy --channel conda-forge
```
...
mamba repoquery search pygypsy --channel conda-forge

# List packages depending on `pygypsy`:
mamba repoquery whoneeds pygypsy --channel conda-forge

# List dependencies of `pygypsy`:
mamba repoquery depends pygypsy --channel conda-forge
```
